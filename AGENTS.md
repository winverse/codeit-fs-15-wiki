...
- 원본 강의 항목 내부에 스크립트가 들어 있으면 같은 번호의 별도 `### NN. #스크립트 제목` 항목으로 분리합니다. 번호 없는 `### #스크립트`를 강의 항목 안에 중첩해 두지 않습니다.
- 같은 번호 안에서는 학생용 본문을 먼저 두고, 그다음 `#스크립트`, `#실습`, `#퀴즈` 순서로 배치합니다.
- `#스크립트`를 제외한 본문, 퀴즈, 실습 안내, 꿀팁은 `합니다/입니다`체의 교과서형 문체로 작성합니다. `다룬다`, `일반적이다`, `확인한다` 같은 `이다/한다`체와 `해볼게요`, `보시면`, `하겠습니다` 같은 말하기 표현은 강사용 대본에만 둡니다.
- 자동 검사는 보조 도구로만 사용하고, 최종 통과 여부는 [강의자료 최종 검수 절차](./docs/00.%20문서%20운영/강의자료%20최종%20검수%20절차.md)에 따라 직접 읽고 판단합니다.
- 중요도 별표는 학생용 본문 개념에만 붙입니다. `#스크립트`, `#실습`, `#퀴즈`에는 기본적으로 붙이지 않습니다.
- `⭐⭐⭐`는 이후 진도를 따라가기 어려워지는 핵심 구조나 필수 문법에만 사용합니다. `⭐⭐`는 자주 틀리는 보조 개념, `⭐`는 특수 문법이나 보충 팁에 사용합니다.
